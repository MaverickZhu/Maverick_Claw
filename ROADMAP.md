# Maverick_Claw 开发路线图

## 项目概览

- **项目名称**: Maverick_Claw
- **版本**: 0.1.0 (MVP)
- **目标**: 面向中国用户的本地/自托管多通道 AI 助手网关
- **技术栈**: Node.js + TypeScript + React + Fastify

## 阶段规划

### Phase 1: MVP 基础功能 (Week 1-8)

#### Week 1: 项目基础设施
- [x] 项目架构设计
- [x] Monorepo 搭建
- [x] 基础类型定义
- [x] 数据库 Schema 设计
- [x] 开发环境完善

#### Week 2: Gateway 核心
- [x] WebSocket 协议完整实现
- [x] 会话管理系统
- [x] 消息存储 (SQLite)
- [x] 认证授权系统

#### Week 3: Web UI 基础
- [x] WebSocket 客户端连接
- [x] 聊天界面功能完善
- [x] 状态管理 (Zustand)
- [x] API 客户端

#### Week 4: AI 模型集成
- [x] DeepSeek Provider
- [x] OpenAI 兼容层
- [x] 模型配置管理（默认模型治理 + 切换策略持久化）
- [x] 流式响应支持

#### Week 5: 通道系统集成
- [x] WebChat 内置通道（会话同步 + 重连恢复 + 错误收敛）
- [x] 微信集成 (wechaty)
- [x] 消息路由系统
- [x] 通道配置管理

#### Week 6: 工具系统
- [x] 工具注册表
- [-] 浏览器控制 (Playwright，延期至 Phase 2)
- [-] 代码执行沙箱（延期至 Phase 2）
- [x] 文件操作工具

#### Week 7: CLI 完善
- [x] CLI 命令完善
- [ ] 配置向导
- [x] 日志查看
- [ ] 调试工具

#### Week 8: 整合测试 & MVP 发布
- [x] 端到端测试
- [x] 性能优化
- [x] 文档完善
- [x] MVP 发布 v0.1.0

### Phase 2: 功能扩展 (Week 9-14)

#### Week 9-10: 更多通道
- [x] 钉钉集成
- [x] 飞书集成
- [x] 企业微信
- [x] 邮件通道

#### Week 11-12: 更多模型
- [x] 文心一言
- [x] 通义千问
- [x] 豆包
- [x] Ollama 本地模型

#### Week 13-14: 高级功能
- [x] 插件系统
- [x] 记忆系统
- [x] 使用统计
- [x] 导入/导出

### Phase 3: 企业级功能 (Week 15+)

- [x] 多用户支持
- [x] 审计日志
- [x] 权限管理 (RBAC)
- [x] 工作流持久化
- [x] SSO/LDAP
- [x] 插件市场

## 当前进度

**当前阶段**: Phase 3 Week 20 已完成

**总体进度**: 100% (Phase 3)

```
Phase 1: [███████████████████] 100%
Phase 2: [███████████████████] 100%
Phase 3: [███████████████████] 100%
```

## 里程碑

| 里程碑 | 日期 | 目标 | 状态 |
|--------|------|------|------|
| M1 | Week 2 | Gateway 可启动，WebSocket 可连接 | ✅ 已完成 |
| M2 | Week 4 | Web UI 可聊天 | ✅ 已完成 |
| M3 | Week 6 | 模型可配置 | ✅ 已完成 |
| M4 | Week 8 | MVP 发布 v0.1.0 | ✅ 已完成 |
| M5 | Week 14 | v0.2.0 功能扩展 | ✅ 已完成 |
| M5.5 | Week 16 | v0.3.0 企业级基础 | ✅ 已完成 |
| M6 | Week 20 | v1.0.0 正式版 | ⚪ 未开始 |

## 当前执行主线（2026-04-14）

### 主线 A：MVP 收口（Phase 1）

1. ✅ 已完成：模型配置管理完善（默认模型管理 + 会话继承 + 前端持久化）
2. ✅ 已完成：WebChat 稳定性加强（会话同步、重连、错误恢复）
3. ✅ 已完成：发布前性能与稳定性回归（基线对比 + 误差预算门禁）
4. ✅ 已完成：文件上传前后端完整链路（multipart + UploadService + HTTP 路由）
5. ✅ 已完成：主题切换（light/dark/system + 持久化）
6. ✅ 已完成：系统命令工具加固（~60 白名单 + 参数化/路径/输出/超时/过滤控制）
7. ✅ 已完成：微信通道集成（wechaty 动态导入 + WeChatAdapter + 消息缓存回复）
8. ✅ 已完成：聊天界面全面优化（消息气泡/空状态/会话标签/Streaming 光标/深色模式）
9. ✅ 已完成：通道适配器重复代码重构（AbstractChannelAdapter 基类 + 公共工具函数 + ChannelResponse 工厂）

### 主线 B：Phase 2 预备

1. ✅ 已完成：钉钉/飞书通道适配接口定稿（统一 webhook 适配器能力契约）
2. ✅ 已完成：通道插件化边界梳理（配置契约校验、鉴权能力声明、消息路由契约）
3. ✅ 已完成：多模型接入标准化（统一 provider 能力矩阵）

### 主线 C：工程质量护栏

1. ✅ 已完成：性能基准纳入常规回归（基线报告 + 趋势对比）
2. ✅ 已完成：错误处理一致性收口（HTTP/WS/Queue 统一规范）
3. ✅ 已完成：类型检查覆盖持续提升（边界 DTO + 协议对象）

### 主线 D：技术债务优先项

1. ✅ 已完成：性能优化首轮（`sessions.list` 聚合计数查询 + benchmark 回归对比）
2. ✅ 已完成：重复代码重构（会话计数回填逻辑上提到 SessionManager，HTTP/WS 错误模板统一，通道适配器基类提取）

## 优先级标记

- 🔴 P0 - 阻塞性，必须完成
- 🟡 P1 - 重要，应该完成
- 🟢 P2 - 期望，可以延后
- ⚪ P3 - 可选，未来版本

## 风险跟踪

| 风险 | 影响 | 概率 | 缓解措施 | 状态 |
|------|------|------|----------|------|
| 微信接口不稳定 | 高 | 中 | 多方案备选 | 🟡 监控中 |
| 开发时间超预期 | 中 | 高 | 功能裁剪机制 | 🟡 可控 |
| 第三方 API 变动 | 中 | 中 | 抽象层隔离 | 🟢 已缓解 |

## 更新日志

- 2026-04-01: 项目初始化，架构设计完成
- 2026-04-14: 根据实际实现校准阶段进度与里程碑状态，补充当前执行主线
- 2026-04-14: 完成模型配置治理（默认模型策略、配置 API、前端持久化与切换收口）
- 2026-04-14: 完成 WebChat 稳定性收口（重连状态机、会话同步、消息回填）
- 2026-04-14: 完成性能回归门禁（benchmark baseline 对比 + error budget 阈值判定）
- 2026-04-14: 完成飞书通道首版接入（事件订阅回调 + URL 验证 + 消息回传）
- 2026-04-14: 完成钉钉通道首版接入（Webhook 回调解析 + 机器人签名回消息）
- 2026-04-14: 完成通道插件化边界梳理（配置校验收口 + 鉴权/路由契约对外查询）
- 2026-04-14: 完成多模型能力矩阵标准化（Provider 能力契约 + HTTP/WS 查询接口）
- 2026-04-15: 完成主线 C 第 1 项（性能基线入库 + CI 常规回归 + 趋势对比归档）
- 2026-04-15: 完成主线 C 第 2 项（统一错误码契约 + HTTP/WS/Queue 错误结构收口）
- 2026-04-15: 完成主线 C 第 3 项（边界 DTO 校验收口 + 协议对象共享类型收口）
- 2026-04-15: 完成主线 D 第 1 项（会话列表计数查询聚合优化，`ws.sessions.list` p95 降低）
- 2026-04-15: 推进主线 D 第 2 项（`SessionManager` 收口会话计数视图，HTTP/WS 去重后基准回归 PASS）
- 2026-04-15: 推进主线 D 第 2 项（HTTP 路由输入校验统一走 `parseRequestInput`，继续收口重复模板代码）
- 2026-04-15: 推进主线 D 第 2 项（`sendInvalidRequestError` 收口 HTTP `sendHttpError` fallback 模板，减少重复错误处理分支）
- 2026-04-15: 完成性能波动隔离复测（同代码同阈值可复现一轮 FAIL 一轮 PASS，`ws.connect` 波动触发门禁；`ws.sessions.list` 在隔离环境与 baseline 接近）
- 2026-04-15: 推进主线 D 第 2 项（新增 `sendNotFoundError`/`sendBadRequestError`，继续收口会话与 webhook 路由重复错误分支）
- 2026-04-15: 推进主线 D 第 2 项（WS `handleRequest` 错误模板收口：validation/notFound/badRequest/unauthorized/forbidden）
- 2026-04-15: 推进主线 D 第 2 项（WS 参数校验统一 `parseWsRequestInput` + default/catch fallback helper 化，并补齐 4 条 E2E 错误分支用例）
- 2026-04-22: Phase 2 Week 13 — 使用统计系统（usage_records + StatsService + Prometheus + Dashboard 真实数据）
- 2026-04-22: Phase 2 Week 13 — 记忆系统持久化（channel_sessions DB 表 + maxContextMessages=20 截断 + system 消息保留）
- 2026-04-22: Phase 2 Week 14 — 导入/导出功能（ExportService/ImportService + ZIP 打包 + API 路由 + UUID 去重）
- 2026-04-22: Phase 2 Week 14 — 插件系统 MVP（Plugin 接口 + PluginManager 动态加载 + PluginContext + deepseek 插件包装）
- 2026-04-22: Phase 3 Week 15 — 多用户系统 + RBAC（users/roles 表 + UserService/RoleService/AuthService + TokenManager DB 化 + 资源所有权隔离）
- 2026-04-22: Phase 3 Week 16 — 审计日志 + 工作流持久化（audit_logs 表 + AuditService + WorkflowService + 自定义工作流 CRUD 与执行）
- 2026-04-22: Phase 3 Week 17 — SSO/OAuth2 + LDAP 认证（openid-client + ldapjs + OAuthService/LDAPService/SSOService + HTTP 路由 + 13 个单元测试）
- 2026-04-22: Phase 3 Week 18 — 插件市场（远程 registry + ZIP 安装/卸载/更新 + PluginMarketService + DB 持久化 + PluginManager DB 驱动加载 + HTTP 路由 + 13 个单元测试）
- 2026-04-22: Phase 3 Week 19 — 前端管理面板 + Auth 体系（登录页 + Auth Store + 路由守卫 + 用户/角色/审计管理页面 + Sidebar 权限菜单）
- 2026-04-22: Phase 3 Week 20 — v1.0.0 正式版收尾（插件市场前端 + 工作流管理前端 + Dashboard Stats 增强 + 版本号升至 1.0.0）
